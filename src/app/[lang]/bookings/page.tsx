...
export default async function BookingsPage({
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 py-8">
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-black">{dict.nav.myBookings}</h1>
        <p className="text-gray-600 mt-1">
          {isChinese ? '查看您已預約的門診' : 'View your scheduled appointments'}
        </p>
      </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-black mb-2">
                  {isChinese ? '如何預約門診' : 'How to Schedule an Appointment'}
                </h3>
                <p className="text-gray-700 mb-4">
                  {isChinese 
                    ? '如需預約諮詢，請直接致電我們的診所。我們的職員將協助您安排合適的時間，並為您安排最適合的醫生。'
                    : 'To book a consultation, please call our clinic directly. Our staff will help you find a suitable time and assign the best doctor for your needs.'
                  }
                </p>
      ...
      {appointments.length === 0 && (
        <Card className="border-gray-200">
          <CardContent className="py-12 text-center">
            ...
            <h3 className="text-lg font-semibold text-black mb-2">
              {isChinese ? '尚無預約' : 'No appointments yet'}
            </h3>
            <p className="text-gray-600">
              {isChinese 
                ? '請致電我們預約您的首次諮詢。預約確認後，您的門診將顯示在此處。'
      ...
      {upcoming.length > 0 && (
        <div className="mb-10">
          <h2 className="text-xl font-semibold text-black mb-4">
            {isChinese ? '即將到來的預約' : 'Upcoming Appointments'}
          </h2>
                ...
                <CardContent className="pt-0">
                  <div className="grid sm:grid-cols-2 gap-4 text-sm">
                    <div className="flex items-center gap-2 text-gray-700">
                      ...
                      {appointment.date.toLocaleDateString(lang === 'zh-TW' ? 'zh-HK' : 'en-US', {
                        weekday: 'long',
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric',
                    ...
                    <div className="flex items-center gap-2 text-gray-700">
                      ...
                      {appointment.date.toLocaleTimeString(lang === 'zh-TW' ? 'zh-HK' : 'en-US', {
                        hour: '2-digit',
                        minute: '2-digit',
                      })}
                    </div>
      ...
      {past.length > 0 && (
        <div>
          <h2 className="text-xl font-semibold text-black mb-4">
            {isChinese ? '過往預約' : 'Past Appointments'}
          </h2>
          <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div>
                <h3 className="font-semibold text-black">
                  {isChinese ? '需要改期？' : 'Need to reschedule?'}
                </h3>
                <p className="text-gray-600 text-sm">
                  {isChinese ? '請致電我們以修改您的預約' : 'Please call us to modify your appointment'}
                </p>
              </div>
              <Button asChild className="bg-black text-white hover:bg-gray-800">
                <a href="tel:+85291061836">
                  <Phone className="w-4 h-4 mr-2" />
                  {isChinese ? '致電診所' : 'Call Clinic'}
                </a>
              </Button>
            </div>
          </CardContent>
